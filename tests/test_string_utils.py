...
class TestNormalizarOutputComando:
    ...
    @pytest.mark.parametrize("input_text,expected", [
        (
            "\x1b[32mSUCCESS\x1b[0m: All tests passed",
...
class TestParsearRutaArchivo:
    ...
    def test_parsea_rutas_correctamente(self, ruta, expected_fields):
        ...
        for field, expected_value in expected_fields.items():
            mensaje = f"Campo {field} no coincide"
            assert resultado[field] == expected_value, mensaje
...
class TestLimpiarNombreArchivo:
    ...
    @pytest.mark.parametrize("nombre,expected", [
        ...
        (
            "file/with\\slashes|and*wildcards",
            "file-with-slashes-and-wildcards"
        ),
...
class TestProcesarSalidaHerramienta:
    ...
    @pytest.mark.parametrize(
        "salida,herramienta,expected_estado,expected_metricas", [
    ...
    def test_procesa_diferentes_outputs(self,
                                        salida,
                                        herramienta,
                                        expected_estado,
                                        expected_metricas):
